...
Page *BufferPoolManager::new_page(PageId *page_id) {
    ...
    const page_id_t new_no = disk_manager_->allocate_page(page_id->fd);
    page_id->page_no = new_no;

    page->reset_memory();
    page->id_.fd = page_id->fd;
    page->id_.page_no = new_no;
    page->pin_count_ = 1;
    page->is_dirty_ = false;
    page_table_[page->id_] = fid;
    replacer_->pin(fid);
    return page;
}
